...
require "vv/decision_object/roi"
...
RSpec.describe Vv::DecisionObject::Roi do
  ...
  def stakes(**overrides)
    Roi::Stakes.build(
      ...
      versus: { [:fast_llm, :human_review] => -420.00 },
      **overrides
    )[:data]
  end
  ...
  describe "build" do
    it "names every problem at once rather than the first" do
      out = Roi::Stakes.build(question: "route", options: {})
    ...
    it "refuses an escape option it cannot price" do
      out = Roi::Stakes.build(question: :route, escape: :nowhere,
  ...
  describe "appraisal" do
    it "prefers the valuable option over the likely one and says so" do
      ...
      expect(a.best).not_to eq :fast_llm
    ...
    it "labels a bounded upside against a long tail as concave" do
      a = appraise({ fast_llm: 0.90, human_review: 0.10 },
                   p: policy(allow_cost_sensitive_selection: true))[:data]
      expect(a.shape).to eq :concave
    end
  ...
  describe "risk appetite" do
    ...
    it "will not let expected value argue past ruin" do
      s = stakes(versus: { [:fast_llm, :human_review] => -50_000.00 })
      a = appraise({ fast_llm: 0.99, human_review: 0.01 },
                   s: s,
                   p: policy(allow_cost_sensitive_selection: true,
                             ruin_below: -5_000.00))[:data]
  ...
  describe "the gate" do
    ...
    it "never upgrades an escalation, however good the numbers look" do
      ...
      expect(a.clears?).to be true
      out = Roi.gate(:escalate, a)
      expect(out[:data][:disposition]).to eq :escalate
    end

    it "leaves a refusal alone" do
      a = appraise({ deterministic_code: 0.99, fast_llm: 0.01 })[:data]
      expect(Roi.gate(:refuse, a)[:data][:disposition]).to eq :refuse
    end
  end

  describe "feedback" do
    it "prices the outcomes the feedback layer already tracks" do
      out = Roi.realize({ resolved: true, reopened: false },
  ...
  describe "portfolio" do
    it "counts the options the system has sold" do
      out = Roi.portfolio([
        { disposition: :commit, reversible: false, worst_case: -250.0, shape: :concave, expected: 1.0 },
        { disposition: :commit, reversible: true,  worst_case: -12.0,  shape: :linear,  expected: 1.0 }
      ])
