...
ux:ComponentShape a sh:NodeShape ;
  ...
  sh:property [ sh:path ux:componentKind ; sh:minCount 1 ; sh:maxCount 1 ;
    sh:in ( ux:PageShell ux:PanelFrame ux:SemanticText ux:StatusBadge ux:MetricStrip
      ...
      ux:EmptyState ux:RefusalNotice ) ] ;
...
| `RefusalNotice` | never-raise typed refusal. Required: `operation` (declared `ux.*`/`meaning.*` name, or kebab-case surface-action id — not trigger prose). Q8: same-document publication is not required; a refusal names an act not on offer, and the page may instead offer a remediation control whose `action` is a different kebab. `ActionControl`/`DecisionForm` `action`, when present, must itself be kebab. Singular `reason` (governing defect), `failedCriteria` (every independent blocker), `remediation`, `overridePolicy`; `evidenceRefs` except for the closed envelope/shape/token-failure reason set. Copyable tree: `Profile9::Acia.conformant_refusal_document`. Authoring: grammar `profile-9-ghis.md` § Authoring `operation`. | all eight |
...
| `ReferentBridge` | one inspectable referent-retention claim. Mandatory: `sourceConcept`, `sourceDefinitionRevision`, `targetExpression`, `mappingArtifact`, `mappingProof`, `sourceToTargetScope`. | translation-review surfaces |
