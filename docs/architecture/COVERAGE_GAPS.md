...
| 7 | `project-graph` | Rails | does not exist | GRAPH volume is **0 triples** (queried). Storable *is* wired in Ruby (`3cbd5bf`); extract compose never sets `MM_OXIGRAPH_URL`, so bootstrap BACK cannot reach oxigraph. `ROLE=project-graph` is a topology question, not why the store is empty. Findings: [`GAP7_GRAPH.md`](GAP7_GRAPH.md). MIND prompt is false; not fixed here | open, investigated |
