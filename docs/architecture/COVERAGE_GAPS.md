...
| 90 | **A consumer resolves from a gem it does not declare** | `profile_catalog.rb` resolves 13 `L8_PROTOCOL_MAP` entries from `shapes-level-8`, but `rails-osi-level-8.gemspec` declares only `rails` + `shapes-application`; L8 arrives transitively. `profile_catalog.rb:86-90` falls back to `repo_root.join("gems", gem_name)`, which EXISTS in the monorepo -- so resolution succeeds declared or not, and can only break once installed outside the tree. `check_shape_gem_deps.py` passed CORRECTLY: its population is 2 gemspecs and the engine is not one of them | nothing checks what a shape-gem CONSUMER declares | delegated to grok |
| 91 | **The protocol engine depends on the application contracts** | `rails-osi-level-8` -> `shapes-application` -> `shapes-level-8`. The inner link obeys the rule application -> protocol, never reversed; the outer one means the engine named for the protocol hard-depends on THIS application -- 18 `SHAPE_MAP` entries resolve into `contracts/mind-pod`. A second application would inherit the mind-pod shapes, the scenario step 1 required to be represented. Counter-argument is real: the engine is the Rails BINDING, not the protocol definition, and a binding may legitimately depend on both | whether the ownership rule governs consumers or only the two shape gems | **owner decision** |
