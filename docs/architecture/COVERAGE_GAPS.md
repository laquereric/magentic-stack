...
| 63 | **NEVER-RAISE HAS NO OBSERVER — pattern, third instance today** | (1) backjob's CPCP push rescued into a printed envelope, completion dead for weeks; (2) `admission_status` said `admitted` regardless of P6; (3) `Publisher::Immediate#schedule` `rescue StandardError → :error`, so `Note.create!` commits while the projection silently does not | the never-raise envelope is right at a boundary, but nothing WATCHES the refusals. Never-raise is becoming never-notice | **owner — systemic** |
