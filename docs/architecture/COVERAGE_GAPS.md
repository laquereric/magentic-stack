...
| 62 | ~~MIND's system prompt is false about the graph~~ | **AUDITED** (TRUE/FALSE/ASPIRATIONAL/**MISCLASSIFIED**). Class docstring is the system prompt (`nooa/agent.py:437-452`). SPARQL and the RDF-as-present-knowledge left out. **"Durable memory" is MISCLASSIFIED** (ADR 0057): the named volume survives restart and dies on `down -v` — ephemeral is the *kind*, not the restart behaviour; volume not changed. BACK/BACKJOB plural in the module docstring. [`GAP62_PROMPT.md`](GAP62_PROMPT.md) | — | closed |
...
| 76 | **"Sole writers" does not say who writes what** | ADR 0056 declares BACK and BACKJOB. Today BACKJOB writes Reconciliation and nothing else -- **my reading of behaviour, not a written rule** | without the split stated and gated, the next write BACKJOB adds is legitimate by precedent rather than by decision | **next** |
| 77 | **WAL is inherited, not declared** | the live domain DB reports journal_mode=wal, but nothing in our tree sets it -- database.yml declares only adapter, pool, timeout:5000. The one PRAGMA journal_mode=WAL is idempotency.rb:41, a DIFFERENT database | under one writer that was luck we did not need; under two it is load-bearing, because rollback-journal mode makes concurrent writes fail far harder. Declare it | **next** |
| 78 | **SQLITE_BUSY would be invisible** | SQLite allows one writer at a time even in WAL; timeout:5000 means the second waits 5s then raises. In BACKJOB that reaches the loop rescue and is PRINTED | the thing we would most want to know about a two-writer arrangement is the least likely to be noticed -- gap 63's pattern arriving at the defect ADR 0056 creates | **next** |
| 79 | **"BACK is the sole writer" is now false everywhere** | singular in routes.rb, the compose header, MIND's system prompt, and ADRs 0052 and 0055 | THIRD time today prose outlived the code: MIND persistence, MIND graph, now this. Wants a sweep, not another patch | **next** |
| 80 | **"Durable memory" MISCLASSIFIES MIND's store** | I wrote it twice at 030af95, in the module docstring and the system prompt. MIND's SQLite is EPHEMERAL NONDETERMINISTIC INFERENCE STATE (ADR 0057) -- a third kind, not a private copy of application state | "durable" invites the model to treat its own store as persistent and truth-adjacent, the inverse of the division. Told grok mid-audit | **in build (gap 62)** |
