...
| 52 | **A P6 denial is stored as `admitted`** | `cpcp_adapter.rb:275` writes `admitted` before `Authorization.admit!` runs; nothing ever writes `refused`. `bin/backjob:42` selects that predicate to push completions, so a denied `note.create` could be completed | **DECIDED (ADR 0052): the column goes, the journal is the only truth.** Derivation = has `authorized`, no `refused` (NOT `response_refused`) | **decided, in build** |
...
| 56 | `l8.execution.complete` rows have an empty journal on their own cid | `p7_commands.rb:151-165` writes the `completed` entry against the PARENT cid and takes `seq` from the parent journal. Structural | **UNBLOCKED for 0052** by declaring the operation `not_an_admission`; still OPEN as a defect — a record that cannot describe itself | open |
| 57 | 2 historical `note.create` rows have neither `authorized` nor `refused` | host `db/mind_pod.sqlite3`; they predate P6. Live volume clean (95/95/0/0) | **DECIDED: backfill** an `authorized` entry MARKED as a backfill — reason, ADR, timestamp — so a reader can always tell no decision was actually made | decided, in build |
| 58 | **`indeterminate` must stay empty** | new fourth state: an operation IN the admission path with neither `authorized` nor `refused`. After the backfill the population is zero | it is an alarm, not a classification — needs a checker, or the next `p7_commands`-shaped bug lands silently as gap 52 did | **next, with 52** |
