...
| delegated | 12, 95, 96 | with grok now |
...
| closed | 1, 2, 3, 14, 16, 21, 24, 42, 44, 47, 51, 52, 53, 54, 55, 56, 57, 58, 60, 62, 63, 66, 67, 71, 74, 76, 77, 78, 79, 80, 64, 65, 88, 89, 90, 93, 92, 61 | 38 rows |
...
| 96 | **`enforced_by` is asserted non-empty, never resolved, and a doc reads like a gate** | gap 92 made `ingest_spec` green by POPULATING `enforced_by` -- the right side to fix, and nothing was invented: **102 refs, 64 unique targets, 63 files + 1 dir, 0 missing.** But that was a one-off manual check by me. `:constraint` means the list is EMPTY, so the spec asserts presence only; `check_adr_kinds.py` never reads `enforced_by`. A rename silently re-breaks it, and a file that cannot enforce anything is indistinguishable from one that does. **Measured:** `0058` (ROLE=LOG) cites `COVERAGE_GAPS.md` -- **the gap table, circular, and I wrote 0058**; `0049` cites a design doc; `0050` cites a bare directory. A middle class DECLARES rather than enforces (`0046/0051/0057` -> compose, `0056/0057` -> `domain_writers.json`) -- 0056 correctly lists the declaration AND `check_two_writers.py`, 0057 lists only declarations. grok states unbuilt targets name a **live stand-in**; that honesty must become machine-visible | a decision reads as enforced because a real file is named next to it | delegated to grok |
