...
| 63 | **NEVER-RAISE HAS NO OBSERVER — pattern, third instance today** | **CENSUS** [`GAP63_CENSUS.md`](GAP63_CENSUS.md): 48 sites (45 `.rb` + 2 `bin/backjob` + 1 pattern bin). 11 FALLBACK, 8 UNCLEAR, 29 REFUSAL. **0 refusals meet the ADR floor.** Mechanism is step 2, not this row | the envelope is right; nothing watches | census ready |
