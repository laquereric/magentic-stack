...
| 88 | **We are inventing an OTEL-to-SHACL vocabulary** | OTEL LogRecords are NOT RDF graphs. Manus flags that the representation is PROPOSED BY ITS MEMO, not derived from OTEL nor mandated by it | "OTEL as the basis" means we author and own a vocabulary at that seam — it is not adoption of an existing standard at that layer | open |
| 89 | **The failure-domain test is not premature** | kill LOG; break the shared Rails image; fill the LOG quota; remove network reachability; restart the producer | expected invariant: the producer's LOCAL FIRST RECORD survives each. This is testable at one pod and worth more than redundant infrastructure | **next** |
