...
| 23 | ~~Shape payload part-migrated (7/59)~~ | **NO USEFUL FRACTION.** 7 is the runtime pin (5 application + 2 protocol). 52 is canonical package + 26 fixtures + ontology + duplicates. Step 10 is **retire** the canonical tree, not move 52 files. SHAPE_MAP never resolves `shapes-level-8`. Findings: [`GAP23_TTL.md`](GAP23_TTL.md) | gap 6 `incomplete:true` reason was wrong | closed as a count, step 10 still unrun |
...
| 63 | **Never-raise has no observer** (ADR 0054) | **CENSUS DONE** (`fab60ef`): 48 sites, not the 45 I counted — my `--include=*.rb` missed extensionless Ruby, including `bin/backjob`, which is the ADR's own instance 1. 11 FALLBACK / 8 UNCLEAR / **29 REFUSAL**. Meeting the ADR floor: **0 of 29**. 22 returned-wire, 3 printed, 2 logged, 2 with no envelope at all, **0 journalled** | step 2 (mechanism) is next and deliberately unstarted | **census done, mechanism open** |
