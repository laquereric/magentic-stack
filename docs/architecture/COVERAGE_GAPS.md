...
| 76 | ~~"Sole writers" does not say who writes what~~ | **CLOSED.** Enumerated from source: BACKJOB writes Reconciliation (+ ProjectionJob via `project_on_save!`); completions are HTTP to BACK. Rule: `config/domain_writers.json`. Gate: `DomainWriters` callbacks + SQL subscriber. Checker: `check_two_writers.py`. Findings: [`GAP76_WRITERS.md`](GAP76_WRITERS.md) | — | closed |
| 77 | ~~WAL is inherited, not declared~~ | **CLOSED.** Live files already `wal` (host sqlite and `mind-pod-demo_mind-data`). Rails 8 adapter `DEFAULT_PRAGMAS` was the source. `database.yml` now declares `pragmas.journal_mode: wal`. Setting WAL on a copy of the live file returns `wal`. **Guarantee delivered; runtime unchanged.** | — | closed |
| 78 | ~~SQLITE_BUSY would be invisible~~ | **CLOSED.** Not a contract change. `SqliteBusy` recognizes the lock timeout; BACKJOB records `reason=sqlite_busy` on RefusalLog instead of a generic print. Plant produces a real `BEGIN IMMEDIATE` collision and shows the JSONL line | — | closed |
| 79 | ~~"BACK is the sole writer" is now false everywhere~~ | **CLOSED as a live-prose sweep.** routes, both compose headers, READMEs, QUICKSTART, bootstrap, rails_cpcp.rb, engine.rb, translation_board.rb. MIND prompt already plural (`b1f871b`). **ADRs untouched** (0056 is the amendment). Dated reviews/plans/profile docs and CPCP "far side is the sole writer" listed, not edited | — | closed |
