...
| 74 | ~~Envelope refusals are not restoration-grade~~ | **CLOSED.** Field list classified against the OTEL log data model. One new attribute key `cpcp.restoration` (four members, absent-or-complete). SHACL only for that object; no `otel:LogRecord` shape. `otel.scope.version` is the schema_version. Dispatcher still omits restoration (envelope has no state). Findings: [`GAP74_RESTORATION.md`](GAP74_RESTORATION.md) | — | closed |
