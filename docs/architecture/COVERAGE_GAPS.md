...
| open | 7, 8, 12, 17, 19, 22, 40, 45, 59, 61, 70, 75, 81, 85, 92, 93 | known, unscheduled |
...
| 93 | **Only one failure reason fails closed; four others still report applied** | `failed_envelope?` in `immediate.rb` and `storable.rb` matches `ok:false` AND `reason == :graph_unreachable` ONLY. `sparql.rb` also returns `:sparql_parse_error`, `:unexpected_error`, `:invalid_graph` and `:invalid_dsl` (sparql.rb:31-35) -- FOUR, I first wrote three and missed `:unexpected_error`. A malformed triples declaration therefore still marks the outbox job `:applied` with nothing in GRAPH -- the exact shape gap 7(b) fixed, surviving for four other reasons. Narrowed deliberately so the engine-limited SPARQL-star annotation DELETE stays non-fatal: that constraint is real, the blanket exclusion is not | a projection can still silently not happen | open |
