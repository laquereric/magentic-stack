...
| 66 | ~~`shapes-level-8` is packaged but unreachable~~ | **CLOSED.** Catalog was the misdescription: manifest already named 13 P11 protocol shapes `execution: runtime` in L8. `L8_PROTOCOL_MAP` resolves them. Gate: `check_catalog_manifest.py`. P9 protocol in L8 stays `execution: none` / canonical source. Findings: [`GAP66_67.md`](GAP66_67.md) | — | closed |
| 67 | ~~P9/P11 differ by comments only~~ | **CLOSED as a record, not a copy.** The row was wrong: P9 also differs in `sh:ignoredProperties` (semantic). Did not strip comments; did not make files identical. Hashes + delta in `packaged_vs_legacy.json`; `check_packaged_legacy.py` fails on any byte change. Plant: minCount 1→2 fails | — | closed |
