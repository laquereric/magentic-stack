...
| 98 | **Catalog IRI and TTL NodeShape IRI can diverge silently** | named by the gap-22 inventory and still true: *no checker compares `Entry#shape_iri` to the TTL NodeShape IRI.* `check_shape_binding` and `check_shape_resolution` key by **local name**; `check_shape_drift` canons `sh:path` to a local token. So a changed catalog IRI, or a changed `@prefix` that keeps local names, **passes every gate**. This is live now, independent of any rename decision | the identity of a record can drift from the identity of the shape that admitted it, with all gates green | delegated to grok |
| 99 | **Nothing gates the one file whose prose IS the product** | MIND system prompt = the `MindCognition` class docstring (NOOA `agent.py:437-452` walks the MRO), **1656 chars today**. No checker in `tooling/` references `mind_agent.py`; `check_boundary.py:158` **explicitly SKIPS docstring lines** (`_docstring_lines`, line 121) -- correct for a code checker, and it means the prose is deliberately excluded; `mind_agent.py` is in no ADR `enforced_by`; `GAP62_PROMPT.md`, which holds the audit, is referenced by **no ADR at all**. Gap 62 found the prompt taught FALSE claims (SPARQL as MIND query surface, knowledge-lives-in-RDF, every-session-ever) and one MISCLASSIFIED. **I wrote "durable memory" into it twice at `030af95`; the owner caught it, not a gate.** This is row 81 made concrete -- accurate-but-categorically-wrong is worse than plainly false, because nothing contradicts it | a one-off audit with no pin: the next edit silently re-teaches what gap 62 removed | delegated to grok |
