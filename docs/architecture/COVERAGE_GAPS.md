...
| 74 | ~~Envelope refusals are not restoration-grade~~ | **CLOSED.** One new attribute key `cpcp.restoration`. SHACL only for that object. `RestorationShape` **excluded** from the OSI L8 census: log attribute, not a protocol payload; lives in `rails-cpcp/shapes`, not either shape gem. Findings: [`GAP74_RESTORATION.md`](GAP74_RESTORATION.md) | — | closed |
