...
Private gem. **Reader only.** The frame and its decisions, as data an agent can
read.
...
and its architecture-decision concepts — and answers structural questions over
it.
...
b.ledger            # {enforced:, unenforced:, ungated:, total:}  the futures gauge
...
| | Forbidden | Grounding |
|---|---|---|
| R1 | summarisation | Nothing rewrites, condenses or paraphrases a decision. Text is served verbatim or by section. A compacted constraint is one the reader never saw the original of — the defect this gem exists to prevent. |
